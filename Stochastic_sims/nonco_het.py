...
max_time = 120000 #maximum timestep
...
repeat_main=int(sys.argv[6]) #number of instances simulated
steady_time=500 #minimum steady state time
...
for z in range(repeat_main):
    ...
    while (time_step < max_time):
        ...
        if time_step==steady+1:
            ...
            virus_1[time_step]+=virus_2[time_step]/2 #changes phage population to be 50:50
            virus_2[time_step]-=virus_2[time_step]/2 #changes phage population to be 50:50
            for j in range(initial_bacteria): #changes type of half of infected cells too
...
result[:,0]=heterozygosity_sum/repeat_main #average heterozygosity as a function of time
result[:,1]=heterozygosity_nonzero_sum/repeat_main #fraction of sims where heterozygosity is non_zero as a function of time

np.savetxt('Het_nonco_'+str(y_1)+'burst'+str(y_2)+'_'+str(alpha)+'alpha'+str(beta)+'_'+str(lyse_time_1)+'tau'+str(lyse_time_2)+'_'+str(sys.argv[1])+'.txt',result)
