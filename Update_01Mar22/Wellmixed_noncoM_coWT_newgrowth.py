# Changes made compared to previous version 'Wellmixed_noncoM_coWT.py'
...
# determine the number of phage released upon lysis in the superinfecting,
# scenario, now the size of the pseudo-populations determine this, that is:
...
for z in range(repeat_main):
    ...
    #Loops through 'repeat' times starting from the steady state?
    ...
    for r in range(repeat):
        ...
        #plt.plot(x,virus_2)
        #plt.show()
    result_main[z][0]=steady_size
    result_main[z][1]=steady_size_error
    result_main[z][2]=mutant
    result_main[z][3]=mutant_fraction
    result_main[z][4]=len(np.where(result==1)[0])
    if (len(np.where(result==1)[0])+len(np.where(result==2)[0]))!=0:
        ...
        result_main[z][1]=-1
    result_main[z][6]=sum(result_time)/len(np.where(result==1)[0])
    np.savetxt('noncoMcoWT_ng_'+str(y_1)+'burst'+str(y_2)+'_'+str(alpha)+'alpha'+str(beta)+'_'+str(lyse_time_1)+'tau'+str(lyse_time_2)+'_'+str(sys.argv[1])+'.txt',result_main,delimiter=' ')
    # np.savetxt('noncoMcoWT_neutral_test_'+str(sys.argv[1])+'.txt',result_main,delimiter=' ')
    # +str(initial_bacteria)+'bacteria_'+str(mutant)+'mutant_'str(lyse_time_2)+'lysis_'
