# Changes made compared to previous version 'Wellmixed_co.py'

# 1. The internal phage populations now grow at rates determined by their
# own parameters, regardless of which phage infected first. The grow is still
# partioned as it was before however, that is:
# virus_1 now grows at rate (virus_1/(virus_1+virus_2))*(y_1/lyse_time_1) and
# virus_2 now grows at rate (virus_1/(virus_1+virus_2))*(y_2/lyse_time_2)
# Beforehand, in the above equations (y_N/lyse_time_N) was the same in both,
# and was set by the initial infecting phage

# 2. A new property of the bacteria is defined: "infect_count"
# This keeps track of the total number of phage that have infected the bacteria
# prior to lysis.

# 3. Rather than using the burst size of the initial infecting phage to
# determine the number of phage released upon lysis, now the size of the
# pseudo-populations determine this, that is:
# the mean of the Poisson distribution is taken to be
# virus_1+virus_2-infect_count at the time of lysis, rather than y_1 or y_2

# 4. Added a timescale parameter "dt" so that the lysis time measurements can be
# run without having to change the hardcoded scales.
...
dt=float(sys.argv[9]) #defines the timescale of a step
time_start = time.time()
time_step=0 #iniitial timestep
max_time=int(4000/dt) #maximum timestep for single loop iteration (will be repeated if fixation/extinction has not occured)
...
delta=0.1*dt #virus decay rate
repeat=int(sys.argv[11]) #number of simulations per steady state reached
repeat_main=int(sys.argv[10]) #number of steady states simulated
steady_time=int(1000/dt) #minimum steady state time
mutant=1 #number of mutants to introduce
t_back=int(30/dt) #used when no fixation/extinction before max time. must be longer than lysis time
...
class Bacteria:
    def __init__(self, location):
        ...
        self.infect_count=0 #keeps track of the number of phage to have infected  before lysis
    ...
    def lyse(self,output_1,output_2,time_step,count_bacteria_alive_bin,j,count_1_infect,count_2_infect):
        if self.primary==2:
            if self.infection_time+lyse_time_2==time_step:
                total_internal=self.virus_1+self.virus_2-self.infect_count
                burst_size=np.random.poisson(total_internal)
...
def infection(bacteria_set,virus_1,virus_2,time_step,count_bacteria_infected,count_bacteria_alive,count_1_infect,count_2_infect):
    ...
    for position in indices_1:
        ...
        bacteria_set[position].infect_count+=1
...
def viral_growth(bacteria_set):
    for position in range(initial_bacteria):
        if bacteria_set[position].infect_count>0:
...
for z in range(repeat_main):
    ...
    np.savetxt('co_ng_'+str(y_1)+'burst'+str(y_2)+'_'+str(alpha)+'alpha'+str(beta)+'_'+str(lyse_time_1)+'tau'+str(lyse_time_2)+'_'+str(sys.argv[1])+'.txt',result_main,delimiter=' ')
time_end = time.time()
