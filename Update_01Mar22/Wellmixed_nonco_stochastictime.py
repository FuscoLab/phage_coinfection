# Changes made compared to previous version 'Wellmixed_nonco.py'

# 1. I have changed the bacteria property "infection_time," which refered to
# the step in which infection initially occured, to "lysis_time," indicating
# the step in which lysis should occur.

# 2. At the point of initial infection, a lysis time "T" is drawn from
# a normal distribution with mean "lyse_time_N" and standard deviation
# "lyse_time_N * width_fraction" where the new parameter "width_fraction"
# is hard coded as 10%, based on experimental measurements
...
width_fraction=0.1 #used to determine the relationship between the mean and stadard deviation of the gaussian distribution of lysis times
...
class Bacteria:
    def __init__(self, location):
        ...
        self.lysis_time = 0 #used to monitor the time lysis should occur
        self.alive = 1
...
def infection(bacteria_set,virus_1,virus_2,time_step,count_bacteria_infected,count_bacteria_alive,count_1_infect,count_2_infect):
    ...
    for position in indices_1:
        ...
        if bacteria_set[position].lysis_time==0:
            T=np.clip(int(np.round(np.random.normal(lyse_time_1, width_fraction*lyse_time_1, 1))), a_max=None, a_min=3) #lysis time drawn from gaussian, clipped to min of 3
            bacteria_set[position].lysis_time=time_step+T
    ...
    for position in indices_2:
        ...
        if bacteria_set[position].lysis_time==0:
            T=np.clip(int(np.round(np.random.normal(lyse_time_2, width_fraction*lyse_time_2, 1))), a_max=None, a_min=3) #lysis time drawn from gaussian, clipped to min of 3
            bacteria_set[position].lysis_time=time_step+T
...
for z in range(repeat_main):
        ...
        while (time_step < max_time):
            ...
            if time_step==(max_time):
                if virus_2[time_step-1]==0 and count_2_infect==0:
                    ...
                    for j in range(initial_bacteria):
                        if bacteria_set[j].lysis_time!=0:
                            bacteria_set[j].lysis_time-=(time_step-(steady+2))
                    time_step=steady+2
    ...
    result_main[z][4]=len(np.where(result==1)[0]) #Number of sims where mutant fixes
    if (len(np.where(result==1)[0])+len(np.where(result==2)[0]))!=0:
        result_main[z][5]=(len(np.where(result==1)[0]))/(len(np.where(result==1)[0])+len(np.where(result==2)[0]))#Fraction of sims where mutant fixes
    else:
        result_main[z][1]=-1
    result_main[z][6]=sum(result_time)/len(np.where(result==1)[0])#Mean time to mutant fixation
    np.savetxt('nonco_st_'+str(y_1)+'burst'+str(y_2)+'_'+str(alpha)+'alpha'+str(beta)+'_'+str(lyse_time_1)+'tau'+str(lyse_time_2)+'_'+str(sys.argv[1])+'.txt',result_main,delimiter=' ')
